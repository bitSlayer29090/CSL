...
TEST_BASEDIR=${NEST_DOC_DIR:-"/cluster/2020shachem/nestInstall7/share/doc/nest"}
...
PYTHON_HARNESS="${NEST_DATA_DIR:-/cluster/2020shachem/nestInstall7/share/nest}/extras/do_tests.py"
...
echo >  "${TEST_LOGFILE}" "NEST v. 2.14.0 testsuite log"
echo >> "${TEST_LOGFILE}" "======================"
...
if test "x${TEST_PYNEST}" = xtrue ; then
    ...
    if command -v nosetests >/dev/null 2>&1 && nosetests --with-xunit --xunit-file=/dev/null --where=${TEST_OUTDIR} >/dev/null 2>&1; then
        ...
        nosetests -v --with-xunit --xunit-file=${TEST_OUTDIR}/pynest_tests.xml ${NEST_PYTHON_PREFIX:-/cluster/2020shachem/nestInstall7/}/nest/tests ${NEST_PYTHON_PREFIX:-/cluster/2020shachem/nestInstall7/}/nest/topology/tests 2>&1 \
