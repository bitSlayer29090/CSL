...
myPl = 1 # if plotting
myM = False # if to_memory for spike detector 'sd'
...
# add '4cores' before .txt if 4 cores
...
for myV in range(1):#len(names)):
	...
	#for myDC in range(len(crashed)):
		...
		sd = nest.Create("spike_detector", 1, {'to_memory':myM})
		nest.Connect(ms, sd)
		print(str(nest.GetStatus(sd)))
		# printing doesnt work
		#print("spike detector recordables: {0}".\
		#	format(nest.GetDefaults(sd)["record_to"]))
