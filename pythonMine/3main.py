...
myMDonlyT = 1 # if writing only times metadata
...
if(myMD):
	...
	metadataFile = open(outputDir + "datsAndMetadata/metadataFiles.txt", "w")
	smf = open(outputDir + "datsAndMetadata/shortMetadataFile.txt", "w")

if(myMDonlyT):
	mdOnlyT = open(outputDir + "datsAndMetadata/metadataOnlyTime.txt", "a")
...
for myV in range(1):#len(names)):
	...
	#for myDC in range(len(didntCrash)):
		...
		if(myMDonlyT):
			mdOnlyT.write("avg seconds spent per nest.Simulate()"\
			" simulation " + str(avg) + "\n")
